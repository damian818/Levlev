...
import { Transaction, DisplayCurrency, InflationPoint } from '../types';
...
import { TrendingUp, X, Filter, Layers } from 'lucide-react';
...
export function CategoryTrendModal({
  ...
  transactions.forEach(t => {
    ...
    const converted = convertCurrency(t.amount, t.currency, displayCurrency, usdArsRate, t.date, transactions, historyData);
    
    // Single category trend
    if (t.category === selectedCategory) {
      monthlyMap[m] = (monthlyMap[m] || 0) + converted;
    }

    // Multi category trend
    if (!multiCategoryMonthlyMap[m]) {
      multiCategoryMonthlyMap[m] = {};
    }
    multiCategoryMonthlyMap[m][t.category] = (multiCategoryMonthlyMap[m][t.category] || 0) + converted;
  });
  ...
  const chartData = sortedMonths.map(m => ({
    ...
    fxRate: getHistoricalFxRate(m, usdArsRate, transactions, historyData),
  }));
      ...
      <div className="bg-[#161b22] border border-slate-800 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl space-y-6 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex justify-between items-center pb-4 border-b border-slate-800">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-100">Historical Trend View by Category</h3>
              <p className="text-xs text-slate-400">Analyze how your spending evolves across months for specific categories.</p>
            </div>
          </div>
              ...
              <BarChart data={stackedChartData}>
                ...
                <Tooltip
                  content={({ active, payload, label }) => {
                    if (active && payload && payload.length) {
                      const data = payload[0].payload;
                      const fxRate = getHistoricalFxRate(String(label), usdArsRate, transactions, historyData);
                      return (
                        <div className="bg-[#161b22] border border-slate-700 p-3 rounded-lg shadow-xl text-xs space-y-1.5">
                          <p className="font-bold text-slate-200">{label}</p>
